...
interface AIPanelProps {
  missedTrades: VertexWsData['missedTrades']
}

type MissedTrade = NonNullable<VertexWsData['missedTrades']>[number]

export default function AIPanel({ missedTrades: missedFromWs }: AIPanelProps) {
  const [missedTrades, setMissedTrades] = useState<MissedTrade[]>(Array.isArray(missedFromWs) ? missedFromWs : [])
  const base = getBotApiBase()
  const api = base ? `${base}/api/trading/ai/missed-trades` : '/api/trading/ai/missed-trades'
