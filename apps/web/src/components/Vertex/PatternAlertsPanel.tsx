...
type PatternDetection = NonNullable<VertexWsData['patternDetections']>[number]

export default function PatternAlertsPanel({ data }: PatternAlertsPanelProps) {
  const [patternDetections, setPatternDetections] = useState<PatternDetection[]>(data?.patternDetections ?? [])
  const base = getBotApiBase()
  const api = base ? `${base}/api/trading/pattern-detections` : '/api/trading/pattern-detections'
            ...
            <table className="compact-table">
              ...
              <tbody>
                {trades.slice(0, 15).map((t, idx) => (
                  <tr key={t.id ?? idx}>
                    <td className="mono">{new Date(t.recorded_at ?? t.entry_time ?? t.timestamp ?? 0).toLocaleString()}</td>
