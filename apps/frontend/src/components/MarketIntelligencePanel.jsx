...
function MarketIntelligencePanel({ marketIntelligence: miFromWs }) {
  const [mi, setMi] = useState(miFromWs);
  ...
  useEffect(() => {
    ...
    fetch('/api/trading/market-intelligence')
      .then((res) => res.json())
      .then((r) => { if (r.status === 'success') setMi(r.data); })
      .catch(() => {});
  }, [miFromWs]);
      ...
      <div className="mi-hero">
        ...
        <div className="mi-metrics">
          <div className="mi-metric">
            ...
            <span className="mi-metric-hint">{mi.put_call_ratio > 1.2 ? 'Bearish' : mi.put_call_ratio < 0.8 ? 'Bullish' : 'Neutral'}</span>
          </div>
          <div className="mi-metric">
