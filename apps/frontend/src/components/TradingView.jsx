...
// All 10 level types for GUI dropdown (chart/user input)
const LEVEL_TYPES = [
...
function TradingView({ data }) {
  ...
  const currentLevels = serverLevels?.levels ?? [];
  const myLevelsForView = manualLevels.filter(
    (l) => l.index === selectedIndex && l.timeframe === selectedTimeframe
  );
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND_URL}/api/trading/ohlc?index=${selectedIndex}&timeframe=${selectedTimeframe}&limit=500`)
      .then((res) => res.json())
      .then((r) => { if (r.status === 'success' && Array.isArray(r.data)) setCandles(r.data); })
      .catch(() => setCandles([]));
  }, [selectedIndex, selectedTimeframe, data?.ohlc?.[selectedIndex]?.[selectedTimeframe]]);

  const buildLevelsPayload = useCallback(() => {
  ...
  const syncLevelsToBackend = useCallback((levelsArray) => {
    fetch(`${BACKEND_URL}/api/trading/levels`, {
      method: 'POST',
  ...
  const removeManualLevel = useCallback((id) => {
  ...
  return (
    <div className="trading-view">
      ...
      <OptionChainPanel
        index={selectedIndex}
        optionChainFromWs={data?.optionChain?.[selectedIndex]}
      />
      ...
      <div className="levels-panel">
        ...
        <div className="levels-file-actions">
          ...
          <a
            href={`${BACKEND_URL}/api/trading/levels/sample`}
            download="levels_sample.csv"
            className="link-download-sample"
          >
            Download sample CSV
          </a>
        </div>
            ...
            <div className="levels-list">
              {currentLevels.map((level, idx) => (
                <div key={`${level.type}-${level.price}-${idx}`} className="level-item">
                  ...
                  <span className="level-confidence">{(level.confidence != null ? level.confidence * 100 : 100).toFixed(0)}%</span>
                </div>
              ))}
            </div>
          ) : (
